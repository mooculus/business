...
] at (current page.center){\reflectbox{\includegraphics[height=8.75in]{starsTurquoise.jpg}}};
...
\hspace*{.4in}\begin{tikzpicture}
  \node[inner sep=10pt,outer sep=0pt, fill=black, text=white, opacity=.8,text opacity=1,rounded corners=0.5cm]  (box){%
    \begin{minipage}{0.50\textwidth}
      ...
      \textbf{The equalities} shown at the bottom are the fundamental
      theorems of calculus for functions mapping to, or from,
      three-dimensional space. For each equation, the message is
      similar:
      \begin{quote}
        \textbf{Accumulation of vectors through a region can be
          computed via vectors on the boundary of the region.}
      \end{quote}
