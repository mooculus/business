...
\begin{image}
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot [very thick, penColor, samples=100,smooth, domain=(-6.75:6.75)] {(x^2)/(4*x)};
...
\begin{question}
  Use the graph of $f(x) = \frac{x^{2}}{4x}$ above to answer the
...
\begin{question}
  Use the graph of $f(x) = \frac{x^{2}}{4x}$ above to finish the following statement: ``A good guess is that\dots''
  \begin{multipleChoice}
    \choice[correct]{$\lim_{x\to 0}\frac{x^{2}}{x} = 0$.}
    \choice{$\lim_{x\to 1}\frac{x^{2}}{x} = 1$.}
    \choice{$\lim_{x\to 1}f(x) = \frac{(0)^{2}}{4}$.}
    \choice{$\lim_{x\to 0}f(x) = \frac{(0)^{2}}{0}=\infty$.}
  \end{multipleChoice}
\end{question}
...
\end{tikzpicture}
\end{image}
If we allow x values on the left of 2 to get close and closer to 2, we see
