...
\begin{question}
  Compute:
  $\lim_{x\to 3} x^2 \begin{prompt}= \answer{9}\end{prompt}$
  \begin{feedback}
    The function $f(x)=x^2$ is of the form $x^k$ for a real number
    $k$.  Therefore, $f(x)=x^2i$ is continuous for all real values of
    $x$.  In particular, $f(x)$ is continuous at $x=3$.  Since $x^2i$
    is continuous at $3$, we know that $\lim_{x\to 3} f(x) = f(3)$.
    That is, $\lim_{x\to 3} x^2 = 3^2$
  \end{feedback}  
\end{question}
