...
\begin{example}
  Compute the following limit using limit laws:
  \[
  \lim_{x \to e} \sqrt{\ln(x)}
  \]
  \begin{explanation}
    By continuity of $x^k$, assuming $\lim_{x \to e} \ln(x) >0$,
    \[
    \lim_{x \to e} \sqrt{\ln(x)} = \sqrt{\lim_{x \to e} \ln(x)},
    \]
    and now since the natural logarithm function is continuous for all $x>0$,
    \begin{align*}
    \sqrt{\lim_{x \to e} \ln(x)} & =\sqrt{\ln(e)}\\
    &= \sqrt{1}\\
    &= 1.
    \end{align*}
  \begin{onlineOnly}
